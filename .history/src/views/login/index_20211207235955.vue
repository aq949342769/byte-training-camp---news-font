<template>
<div>
  <van-nav-bar 
    left-text="返回"  
    left-arrow
    @click-left="$router.back()"
  />
  <div class="container">
    <div class="LoginBox">
      <div class="LoginBox_title">密码登录</div>
      <div class="SignFlow LoginBox_content">
        <div class="username">
          <van-field v-model="userinfo.username" name="账号" placeholder="请输入注册邮箱或账号" :rules="[{ pattern, message: '请输入正确内容' }]" />

        </div>
        <!-- <van-field v-model="userinfo.username" name="账号" placeholder="请输入注册邮箱或账号" /> -->
        <van-field v-model="userinfo.password" type="password"  placeholder="请输入密码" />
      </div>
      <div class="Login_options">
        <button type="button" class="Button Login_goRegist Button--plain" @click="goRegister">立即注册</button>
        <button type="button" class="Button Login_forgetPassword Button--plain" @click="goRetrieve">忘记密码</button>
      </div>
      <button type="submit" class="Button LoginBox_submitButton Button--yellow" @click="handleLogin">登录</button>
    </div>
  </div>
</div>
</template>

<script>
import { useRouter } from "vue-router";
import { useStore } from "vuex";
import { reactive } from "vue";
import { Toast } from "vant";

const loginTips = (tip) => {
    Toast({message: tip});
};

const loginToast = () => {
  Toast.loading({
    message: "加载中...",
    forbidClick: true,
  });
}
export default {
  setup() {
    const store = useStore();
    const router = useRouter();
    const userinfo = reactive({
      username: "",
      password: ""
    });

    const handleLogin = () => {
  
      
      /* store.dispatch("user/handleLogin", userinfo);  */
    };

    const goRegister = () =>{
      loginToast();
      setTimeout(() => {
        router.push({ name: "register" });
        Toast.clear();
      }, 500);
    };

    const goRetrieve = () =>{
      loginToast();
      setTimeout(() => {
        router.push({ name: "retrieve" });
        Toast.clear();
      }, 500);
    };
    
    return {
      userinfo,
      handleLogin,
      goRegister,
      goRetrieve
    }
  }



}
</script>

<style lang="less">
@import "../../assets/css/SignFlow.less";
body {
  height: 100%;
}
</style>>


