  ...
  <div class="container">
    <div class="LoginBox">
      ...
      <div class="SignFlow LoginBox_content">
        <van-field @input="checkNull(userinfo.username,unfillAccount)" v-model="userinfo.username" name="账号" placeholder="请输入注册邮箱或账号" :error="unfillAccount" />
...
const doVerify = () => {
  const accountVerify = (value, isErr) => {
    if(!value){
        isErr.value = true
    }
  }
  ...
  return{accountVerify, checkNull}
}
...
export default {
  setup() {
    const {accountVerify,checkNull} = doVerify();
    ...
    const handleLogin = () => {
      accountVerify(userinfo.username, unfillAccount)
